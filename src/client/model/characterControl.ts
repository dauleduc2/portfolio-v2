...
export class CharacterControls {
    ...
    public update(delta: number, keysPressed: any) {
        ...
        if (this.currentAction == 'Running' || this.currentAction == 'Walking') {
            ...
            const rotateDeg = this.rotateDeg(keysPressed)
            // rotate model
            this.rotateQuarternion.setFromAxisAngle(
                this.rotateAngle,
                angleYCameraDirection + rotateDeg
            )
    ...
    private rotateDeg(keysPressed: any) {
        ...
        if (keysPressed[W]) {
            ...
            if (keysPressed[A]) {
                directionOffset = -Math.PI + Math.PI / 4 + Math.PI / 2 // s+a
            } else if (keysPressed[D]) {
                directionOffset = -Math.PI - Math.PI / 4 - Math.PI / 2 // s+d
            } else {
                directionOffset = 0 // s
            }
        } else if (keysPressed[A]) {
